...
```{r}
election_dat30 <- readRDS("../historic/2023-10-02/30.rds") %>%
      # left_join(all_dat) %>% 
    rename(internal_id = page_id) %>% 
  filter(party != "And")  %>% 
  filter(is.na(no_data)) 

election_dat7 <- readRDS("../historic/2023-10-02/7.rds") %>%
      # left_join(all_dat) %>% 
    rename(internal_id = page_id) %>% 
  filter(party != "And")  %>% 
  filter(is.na(no_data)) 
...
#   filter(location_type == "countries") %>% 
#   arrange(internal_id)
...
#         filter(party == "SMER") %>% 
#         filter(type == "location") %>%
#         # count(location_type, internal_id, sort  =T)
...
#         arrange(desc(spend_per)) %>% 
#         slice(1) %>% ungroup() %>% summarize(spend_per = sum(spend_per))
```
...
region_dat <- election_dat30 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Slovakia")) 
